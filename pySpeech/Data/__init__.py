...
from Autodesk.Revit.DB import ElementId, FilteredElementCollector, BuiltInCategory, BuiltInParameter, Transaction, TransactionGroup, View
...
class ViewWorker(DataWorker):
	...
	def SetAfterCopy(self, view, name, res):
		newname = res["prefix"] + " " + name + " " + res["suffix"]
		try:
	...
	def SplitPrefix(view):
		name = view.LookupParameter(property_names.view_name).AsString()
		index = name.find(" ")
		prefix = name[:index]
